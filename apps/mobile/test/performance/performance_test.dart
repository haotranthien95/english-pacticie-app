  ...
  group('Performance Benchmarks', () {
    testWidgets('App startup time should be < 3 seconds', (tester) async {
      ...
      expect(
        ...
        true,
        reason: 'App startup took ${startupDuration.inMilliseconds}ms, expected < 3000ms',
      );
    });
    ...
    testWidgets('List scrolling should be smooth with 100+ items', (tester) async {
      ...
      expect(
        ...
        true,
        reason: 'Scrolling took ${scrollDuration.inMilliseconds}ms, expected < 2000ms',
      );
    });
    ...
    testWidgets('Button tap response time should be < 100ms', (tester) async {
      ...
      expect(
        ...
        true,
        reason: 'Button response took ${responseDuration.inMilliseconds}ms, expected < 100ms',
      );
    });
  ...
  group('Performance Regression Tests', () {
    testWidgets('Rapid button taps should not cause lag', (tester) async {
      ...
      expect(
        ...
        true,
        reason: 'Rapid taps took ${totalDuration.inMilliseconds}ms, expected < 3000ms',
      );
    });

    testWidgets('Multiple screen transitions should not slow down', (tester) async {
      app.main();
      await tester.pumpAndSettle();
      await tester.pump(const Duration(seconds: 2));
      await tester.pumpAndSettle();
      ...
      if (transitionTimes.length >= 10) {
        final firstFive = transitionTimes.take(5).map((d) => d.inMilliseconds).toList();
        final lastFive = transitionTimes.skip(15).take(5).map((d) => d.inMilliseconds).toList();
        ...
        expect(
          ...
          true,
          reason: 'Performance degradation detected: first ${avgFirst}ms, last ${avgLast}ms',
        );
      }
    });
    ...
    testWidgets('Text input should be responsive', (tester) async {
      ...
      expect(
        ...
        true,
        reason: 'Text input took ${inputDuration.inMilliseconds}ms, expected < 500ms',
      );
    });
  });
...
Future<Duration> measureAsyncOperation(Future<void> Function() operation) async {
  final startTime = DateTime.now();
  await operation();
